...
Model_111$residuals #estimated - observed. we want residuals to be white noise centered at 0 with no time dependence between the error. the error at t should not depend on error at t-1. ACF and PACF should have insignificant coefficients always between the two dashed lines.
Model_111$fitted
plot_grid(
...
#Now let's try ARIMA(0,1,1) # no AR component. differencing the data and adding a moving average term.
...
compare_aic
#remember that the middle term always has to stay 1 because we are doing the differencing. we want the aic to be the lowest as possible. so far, the 011 is the best fit because the number is the lowest.
...
#the 212 is the best because it has the lowest AIC value.
```
...
#You will get an error! it can't fit the series because you need the differencing.
...
```{r}
#plot the observed series vs the fitted series that we obtained with the model
