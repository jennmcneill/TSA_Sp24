...
ggplot(inflow_data, aes(x=my_date))+
  geom_line(aes(y=HP1), color="navy")+
  geom_line(aes(y=HP2), color="tomato2")+
  xlab("Year")+
  ylab("HP1 (cubic m per second)")
...
```{r}
## time series objects can only take numbers. dataframes can have data of other formats all together.
## we want all the rows, so we leave it blank before the comma
## we want the second column, so put a 2 after the comma
...
```{r acf}
## both of these options will give you the same results, but the plots will look a little bit different depending on which one you use

# Option 1: The function acf() and pacf() is from package "stats"
## autocorrelation function that gives us information about the self-correlation of our time series
## how one observation is correlated with a different observation of the same variable
## the output will be a plot that helps you visualize the autocorrelation better
HP1_acf <- acf(ts_inflow_data)
## we see that a seasonal component is present in your data set because the acf is sinusoidal/periodic
## calling this with the $ will print the values in the console
HP1_acf$acf
