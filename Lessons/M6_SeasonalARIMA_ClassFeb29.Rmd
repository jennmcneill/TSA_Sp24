...
Our data goes from January 2001 to November 2022. I will remove a couple rows randomly and show you how to detect missing rows.
...
# the original PACF with a lag close to 1 means that you might have unit root which indicates stochastic trent
...
#we don't need to include s=12 because we already specified the seasonality when we created our timeseries.

print(Model1)
plot_grid(
  autoplot(Model1$residuals),
  autoplot(Acf(Model1$residuals, lag = 40, plot=FALSE)),
  autoplot(Pacf(Model1$residuals, lag = 40, plot=FALSE)),
  nrow=1
)
...
plot_grid(
  ...
  autoplot(Acf(Model2$residuals, lag = 40, plot=FALSE)),
  autoplot(Pacf(Model2$residuals, lag = 40, plot=FALSE)),
  nrow=1
)
#we know the auto is a better fit because the AIC is -178 and we got -177. we want the one that has the most negative AIC value.
...
#recall from M6 that the best fit for the non-seasonal time series is an ARIMA(2,1,2) with drift
