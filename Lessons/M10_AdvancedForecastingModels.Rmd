...
#no NAs. if you still have NA after aggregation, then you need to use the TS Clean function.
```
...
<<<<<<< HEAD
                            start=c(2006,12,16)) 
#hour may need to be specified because the df starts at hour 17 on 12/16
=======
                            start=c(2006,12,16)) #specify the hour
>>>>>>> ea5c000d0d54f7ef2e533c1cf33f9b59d52bffb1
...
autoplot(ETS_fit) + ylab("Active Power") + theme_light()
...
#seasonal=false because you have to say it's not seasonal when you have multiple seasonalities. it can't handle multiple, so we are adding external regressors to account for the different seasonalities. the fourier() function allows us to provide information about our different seasonalities that we want to consider in the forecast. K() means that to model the seasonal weekly pattern, we only need 2 terms. one for week days, one for weekend. The 12 shows the number of months. aka these are the minimum numbers of harmonics that we might need. You can try changing the K and seeing if you get a better fit. The fewer terms, the better the model. autoarima doesn't give you forecast, it just gives you the fit of the ts you have.
...
#also need to specify h for fourier terms. the h= is showing that you want 365 forecasts of the forecast function. this gives us forecast for the fourier terms. if you want to change the (2,12) you need to change it above too. h= and h= comes twice because one is for the forecast function and the other is for the fourier function. **the fit and the forecast come separately when using the autoarima!!!**
...
#Plot foresting results
autoplot(ARIMA_Four_for) + ylab("Active Power") + theme_light()
...
## in the xreg argument you would add the cbind argument and bind any other regressor like another variable
## when you call the forecast function you also need to make sure that you are forecasting the regressors as well, that's where the "h" comes into play
