...
model_1 <- meanf(ts_electricity_price[,"All.sectors"], h=12)
summary(model_1)
plot(model_1)

#Model 2: Arithmetic mean on deseas data
model_2 <- meanf(deseasonal_allsectors_price, h=12)
summary(model_2)
plot(model_2)

#Model 3: Seasonal naive on original data
model_3 <- snaive(ts_electricity_price[,"All.sectors"], h=12)
summary(model_3)
plot(model_3)

#Model 4: Naive on deseas data
model_4 <- naive(deseasonal_allsectors_price, h=12)
summary(model_4)
plot(model_4)
...
model_5 <- sma(deseasonal_allsectors_price, h=12) #, holdout=TRUE)
autoplot(deseasonal_allsectors_price, series="Deseasonal")+
  autolayer(model_5$forecast,series="SMA")

# Model 6: Simple moving average on original data
model_6 <- sma(ts_electricity_price[,"All.sectors"], h=12)
autoplot(ts_electricity_price[,"All.sectors"], series="Original")+
  autolayer(model_6$forecast,series="SMA")

# Model 7:  Simple exponential smoothing on deseasonal data
model_7 <- ses(deseasonal_allsectors_price, h=12)
plot(model_7)
autoplot(deseasonal_allsectors_price, series="Deseasonal")+
  autolayer(model_7$fitted,series="SMA")

# Model 8:  Simple exponential smoothing on original data
model_8 <- ses(ts_electricity_price[,"All.sectors"], h=12)
plot(model_8)
autoplot(ts_electricity_price[,"All.sectors"], series="Original")+
  autolayer(model_8$fitted,series="SMA")
...
model_9 <- auto.arima(ts_electricity_price[,"All.sectors"])
model_9_forecast <- forecast(model_9, h=12)
plot(model_9_forecast)
autoplot(ts_electricity_price[,"All.sectors"], series="Original")+
  autolayer(model_9$fitted,series="SARIMA")+
  autolayer(model_9_forecast$mean,series="SARIMA_forecast")


# Model 10:  ARIMA on deseasonal data
model_10 <- auto.arima(deseasonal_allsectors_price, seasonal=FALSE)
model_10_forecast <- forecast(model_10, h=12)
plot(model_10_forecast)
autoplot(deseasonal_allsectors_price, series="Deseasonal")+
  autolayer(model_10$fitted,series="SARIMA")+
  autolayer(model_10_forecast$mean,series="SARIMA_forecast")
